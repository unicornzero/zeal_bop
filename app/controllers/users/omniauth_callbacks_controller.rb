class Users::OmniauthCallbacksController < Devise::OmniauthCallbacksController
  ...
  def connect
    ...
    authentication = provider.user_authentications.where(uid: auth_params.uid, user_id: current_user.id).first

    if authentication
      authentication.update_from_omniauth(auth_params)
      redirect_to apps_url, flash: {notice: "Your connection to #{provider.name} has been updated."}
    else
      UserAuthentication.create_from_omniauth(auth_params, current_user, provider)
      redirect_to apps_url, flash: {notice: "Your connection to #{provider.name} has been created."}
    end
  end
